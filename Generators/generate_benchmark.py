...
def load_weights(i: int, m: int, p: int) -> list:
    """
    ...
    if m < 2:
        with open(f'Data/weights/w_{i}_{m}_{p}.csv', 'r') as w_f:
            lines = w_f.readlines()
            return [np.array([l.strip('\n').split(',') for l in lines], dtype=float)]
    else:
        with (open(f'Data/weights/fc1_{i}_{m}_{p}.csv', 'r') as fc1,
              open(f'Data/weights/fc2_{i}_{m}_{p}.csv', 'r') as fc2):
...
def main(index: int, model: int, precision: int):
    """
    ...
    with open(f'Data/points/xv_{index}_{model}_{precision}.csv', 'r') as xv_f:
        lines = xv_f.readlines()
        xv = np.array([l.strip('\n').split(',') for l in lines], dtype=float)

    # Create property
    eps = 10 ** -precision
    with open(f'Data/labels/yt_{index}_{model}_{precision}.csv', 'r') as lab:
        l = int(lab.read())
